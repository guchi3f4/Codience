...
RSpec.describe 'ユーザーログイン後のArticle関連のテスト', type: :system do
  ...
  describe '記事一覧画面のテスト' do
    ...
    describe 'サイドバーのテスト' do
      ...
      context '新規投稿フォームの表示' do
        ...
        it 'titleフォームに値が入っていない' do
          expect(find_field('article[title]').value).to be_blank
        end
        it 'Linkフォームが表示される' do
          expect(page).to have_field 'article[link]'
        end
        it 'Linkフォームに値が入っていない' do
          expect(find_field('article[link]').value).to be_blank
        end
        ...
        it 'Summaryフォームに値が入っていない' do
          expect(find_field('article[summary]').value).to be_blank
        end
    ...
    describe '投稿一覧のテスト' do
      context '自分の投稿を表示' do
        ...
        it '画像が表示される' do
          expect(page).to have_selector "img[src$='#{article.user.profile_image.id}/profile_image']"
        end
        it '画像のリンク先が正しい' do
      ...
      context '他人の投稿を表示' do
        it '画像が表示される' do
          expect(page).to have_selector "img[src$='#{other_article.user.profile_image.id}/profile_image']"
        end
        it '画像のリンク先が正しい' do
  ...
  describe '新規投稿画面のテスト' do
    ...
    context '新規投稿フォームの表示' do
      ...
      it '「投稿フォーム拡大」と表示されない' do
        expect(page).not_to have_link '投稿フォーム拡大', href: new_article_path
      end
      it 'titleフォームが表示される' do
        expect(page).to have_field 'article[title]'
      end
      ...
      it 'bodyフォームが存在する' do
        expect(page).to have_selector '#article_form_body', visible: false
      end
      it 'bodyフォームに値が入っていない' do
        expect(find('#article_form_body', visible: false).value).to be_blank
      end
      it 'Create articleボタンが表示される' do
        expect(page).to have_button 'Create Article'
      end
    ...
    context '投稿成功のテスト', js: true do
      before do
        ...
        find("#tag-field").send_keys :enter
        fill_in 'article[summary]', with: Faker::Lorem.characters(number: 20)
        find("span.article_form_arrow").click
        fill_in 'article[body]', with: Faker::Lorem.characters(number: 30)
      end
  ...
  describe '記事編集画面' do
    ...
    context '投稿編集フォームの表示' do
      it '「Editing Article」と表示される' do
      ...
      it 'titleフォームに値が入っている' do
        expect(find_field('article[title]').value).to eq article.title
      end
      it 'Linkフォームが表示される' do
        expect(page).to have_field 'article[link]'
      end
      it 'Linkフォームに値が入っている' do
        expect(find_field('article[link]').value).to eq article.link
      end
      it 'Categoryフォームが表示される' do
        expect(page).to have_field 'category_name'
      end
      it 'Categoryフォームのセレクトボックスに投稿したカテゴリが表示される' do
        expect(page).to have_selector 'select', text: article.category.name
      end
      it 'Tag検索フォームが表示される（入力用）', js: true do
        expect(page).to have_field 'tag-field'
      end
      ...
      it '非表示のTag検索フォームに値が入っている', js: true do
        expect(find('#tag-names', visible: false).value).to eq tag.name
      end
      it 'Summaryフォームが表示される' do
        expect(page).to have_field 'article[summary]'
      end
      it 'Summaryフォームに値が入っている' do
        expect(find_field('article[summary]').value).to eq article.summary
      end
      it 'bodyフォームが存在する' do
        expect(page).to have_selector '#article_form_body', visible: false
      end
      it 'bodyフォームに値が入っている' do
        expect(find('#article_form_body', visible: false).value).to eq article.body
      end
      it 'Update Articleボタンが表示される' do
    ...
    # context '編集成功のテスト', js: true do
    #   before do
    #     @old_title = article.title
    #     @old_link = article.link
    #     @old_category = article.category.name
    #     @old_tag = tag.name
    ...
    #   end
    #   it 'linkが正しく更新される' do
    #     expect(article.reload.link).not_to eq @old_link
    #   end
    #   it 'category正しく更新される' do
    #     expect(article.reload.category.name).not_to eq @old_category
    #   end
    #   it 'tagが正しく更新される' do
    #     expect(Tag.last.name).not_to eq @old_tag
    #   end
    #   it 'summaryが正しく更新される' do
    #     expect(article.reload.summary).not_to eq @old_summary
    #   end
    #   it 'bodyが正しく更新される' do
    #     expect(article.reload.body).not_to eq @old_body
    #   end
    #   it '遷移先が、保存できた投稿の詳細画面になっている' do
    #     expect(current_path).to eq article_path(article)
    #     expect(page).to have_content 'Article Detail'
